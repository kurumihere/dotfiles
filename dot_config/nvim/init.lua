...
local function read_first_line(path)
	...
	return vim.trim(lines[1]):lower()
end

local function system_prefers_dark()
	local theme_switch = read_first_line("~/.config/theme-switch/current")
	if theme_switch == "dark" then
		return true
	elseif theme_switch == "light" then
		return false
	end

	if vim.fn.executable("gsettings") == 1 then
		local color_scheme = table.concat(vim.fn.systemlist({
			"gsettings",
			"get",
			"org.gnome.desktop.interface",
			"color-scheme",
		}), "\n"):lower()
		if color_scheme:find("prefer%-dark") then
			return true
		elseif color_scheme:find("prefer%-light") or color_scheme:find("default") then
			return false
		end
	end

	for _, gtk_settings in ipairs({ "~/.config/gtk-4.0/settings.ini", "~/.config/gtk-3.0/settings.ini" }) do
		local ok, lines = pcall(vim.fn.readfile, vim.fn.expand(gtk_settings))
		if ok then
			for _, line in ipairs(lines) do
				local normalized = vim.trim(line):lower()
				if normalized == "gtk-application-prefer-dark-theme=1" then
					return true
				elseif normalized == "gtk-application-prefer-dark-theme=0" then
					return false
				end
			end
		end
...
local function apply_system_theme()
	local is_dark = system_prefers_dark()
	local next_theme = is_dark and "dark" or "light"
	if current_theme == next_theme then
		return
	end
...
vim.opt.cmdheight = 0 -- hide built-in command line
...
local function rename_tmux_window()
	...
	vim.system({ "tmux", "rename-window", "vim (" .. filename .. ")" }, { detach = true })
end

local function restore_tmux_window_automatic_rename()
	if not in_tmux() then
		return
	end
...
vim.api.nvim_create_user_command("TrimWhitespace", function()
	...
	vim.notify("Trailing whitespace removed", vim.log.levels.INFO, { title = "Neovim" })
end, { desc = "Trim trailing whitespace" })

vim.keymap.set("n", "<leader>tw", "<cmd>TrimWhitespace<CR>", { desc = "Trim trailing whitespace" })
...
-- wrap long prose files without enabling spellcheck by default
...
vim.api.nvim_create_autocmd("VimLeave", {
	group = augroup,
	callback = restore_tmux_window_automatic_rename,
})
